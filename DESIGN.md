...
| Device surface | `surface-device` | `#EAF0F7` | Main device card, distinguished from the screen |
...
|------|-------|--------------------------------------------|------------------|-------|
| Compact status | `lv_font_harmonyos_status_10` | HarmonyOS Sans SC Regular (ASCII only) | 11 px | Main S/P and rate/depth badges only |
...
- Standard Chinese text roles remain 12, 14, and 16 px. User-requested exception:
  `lv_font_harmonyos_status_10` is a 10 px ASCII-only role for S/P markers and
  the compact rate/depth badges, generated by `docs/tools/gen_status_font.ps1`.
  Do not use it for body text, Chinese labels, buttons or other screens.
...
- Icon color stays neutral; focus and selection are expressed by row
  ...
  Exception: main-screen transport icons use semantic colors; USB icons use
  blue recoloring to match the rate/depth badges (gray when off; see Main
...
### Main dashboard (compact left-card layout)
- Preserve the original two-column composition: pale #EAF0F7 96x70 device surface at
...
- Four groups share the bottom line: transport (14 px WiFi/BLE icon), `S` + four ascending signal bars,
  ...
  The bar groups both mean more is better. Empty bars use border-subtle.
- Packet integrity: 0% loss = four green bars; 1-2% = three amber bars;
  3-9% = two amber bars; 10-99% = one red bar; 100% = zero bars with a red P.
  P shares the state color so total packet loss remains visibly red.
- Right status group stays on ONE row at y=4, height 16: sample rate at
...
- Empty state is a bounded label in the left content region; settings remains
  reachable. No battery/loss percentages or transport icon in the right badges.

### Reconnect status chip
- Non-modal 94x48 warning-surface panel covers the left card at (1,21),
  below the tabs, leaving settings reachable. Its bounded 90 px label has
  two lines so `设备255` and `重连中...` remain readable without truncation.
- Multiple devices show `重连中... xN`; the chip disappears once every
  pending device reconnects or is removed. It never joins an encoder group.
- Removing a card also removes its tab button and keeps the active index valid.
...
- Body text stays at least 12 px; only the explicitly requested compact ASCII
  dashboard annotations use 10 px. Focus is indicated by more than a subtle
